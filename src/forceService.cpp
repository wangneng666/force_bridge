...
int forceService::start() {
    ...
    //阻抗插件初始化
//    force_plugin->setForcePlugin("hsImpenderrForce","1", "/home/wangneng/work/hirop/src/force/config/parmtemplate.yaml");
    force_plugin->setForcePlugin("hsImpenderrForce","1", "");
    ...
    //启动服务
    impedenceStart_server = Node.advertiseService("impedenceStart", &forceService::impedanceStartCB,this);
    initRosTopic();
    initRosServer();
    return 0;
}

void forceService::initParam() {
...
void forceService::foruceCallbackXZ(const geometry_msgs::Wrench::ConstPtr &msg) {
...
void forceService::foruceCallbackZX(const geometry_msgs::Wrench_<allocator<void>>::ConstPtr &msg) {
...
//    ready_stop_sub = Node.subscribe<std_msgs::Bool>("set_ready_exit", 1, &forceService::set_ready_callback,this);
    ...
    if(!XZReverseDirect)
        force_sub = Node.subscribe("/daq_data", 1, &forceService::foruceCallbackXZ, this);
    else
        force_sub = Node.subscribe("/daq_data", 1, &forceService::foruceCallbackZX, this);
}

void forceService::initRosServer(){
//    serverControl = Node.advertiseService("impedance_control", &forceService::impedanceStartControl,this);
}
